...
class Bootstrap {
	...
	public function enqueue_script(): void {
		...
		\wp_enqueue_style( 'power-shop-front', Plugin::$url . '/legacy/inc/assets/css/front.min.css', [], Plugin::$version );
		// if (!\is_admin() && ($screen->id !== Plugin::$kebab)) return;
		Vite\enqueue_asset(
			Plugin::$dir . '/legacy/js/dist',
			'js/src/main.tsx',
			[
				'handle'    => Plugin::$kebab . '-legacy',
				'in-footer' => true,
			]
		);
		...
		// 获取目录中的所有文件
		$files = glob( Plugin::$dir . '/legacy/js/dist/assets/*.css' );

		// 遍历文件并使用wp_enqueue_style加载它们
		foreach ( $files as $file ) {
			$file_url = Plugin::$url . '/legacy/js/dist/assets/' . basename( $file );
	...
	public function enqueue_flipdown_script(): void {
		\wp_enqueue_script( 'flipdown', Plugin::$url . '/legacy/inc/assets/packages/flipdown/flipdown.min.js', [], Plugin::$version, false );

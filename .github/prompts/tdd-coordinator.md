...
3. 以 TDD 流程實作功能：
   - 先寫測試（使用對應的 aibdd skill 或直接補 Playwright E2E）
   - 再寫生產程式碼讓測試通過
   - 重構
4. 所有變更 commit 到當前分支
...
- 動態組裝你需要的 agent team（使用 Agent tool），根據規格內容決定：
  - PHP 後端（REST API、Domains/）：使用 aibdd.auto.php.it.* skills
  - TypeScript API / React 前端：使用 aibdd.auto.typescript.* skills
  - 前端 E2E：使用 aibdd.auto.typescript.e2e.* skills 或直接撰寫 Playwright 測試（放在 tests/e2e/ 對應目錄）

...
class CPT extends Bootstrap {
	...
	const VAR          = 'ps_report';
	const POST_META    = array( 'meta', 'settings' );
	...
	public function __construct() {
		...
		\add_filter( 'query_vars', array( $this, 'add_query_for_report' ) );
		\add_filter( 'template_include', array( $this, 'load_report_template' ), 999 );

		\add_action( 'wp_insert_post', array( $this, 'set_default_power_shop_meta' ), 10, 3 );

		\add_action( 'publish_' . self::CPT_SLUG, array( $this, 'post_published_limit' ), 999, 3 );
		\add_filter( 'post_row_actions', array( $this, 'remove_row_actions' ), 999, 2 );
		\add_filter( 'bulk_actions-edit-' . self::CPT_SLUG, array( $this, 'remove_bulk_actions' ), 999, 1 );
	...
	public function init(): void {
		...
		// 新增 {self::KEBAB}/{slug}/report 網址規則
	...
	public function add_metaboxs(): void {
		Functions::add_metabox(
			array(
				'id'    => self::RENDER_ID_1,
				'label' => __( 'Added Products', self::KEBAB ),
			)
		);
		Functions::add_metabox(
			array(
				'id'    => self::RENDER_ID_2,
				'label' => __( 'Sales Stats', self::KEBAB ),
			)
		);
	}

	/**
	 * 設定 {self::KEBAB}/{slug}/report 的 php template
	 */
	...
	public function set_default_power_shop_meta( $post_id, $post, $update ) {
		...
		// 剛創建時，且 post type === self::KEBAB
		if ( ! $update && $post->post_type === self::CPT_SLUG ) {
			// Add default post_meta
			$default_password   = \wp_create_nonce( self::KEBAB );
			$encrypted_password = base64_encode( $default_password );

			\add_post_meta( $post_id, self::SNAKE . '_report_password', $encrypted_password, true );
			\add_post_meta( $post_id, self::SNAKE . '_meta', '[]', true );
		}
	}

...
const ENDPOINT = 'power-shop/reports/dashboard/trend'
...
test.describe('Dashboard 營收趨勢圖表（端點尚未實作）', () => {
  let nonce: string
  ...
  // ─── 端點不存在：帶時序參數請求仍應回傳 404 ───

  test('帶 interval 與日期參數請求仍應回傳 404', async ({ request }) => {
    const res = await apiGet(request, {
      interval: 'day',
  ...
  test('404 錯誤回應應符合 WordPress REST API 標準錯誤格式', async ({ request }) => {
    const res = await apiGet(request, { interval: 'month' })

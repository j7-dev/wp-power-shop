...
const ENDPOINT = 'power-shop/reports/revenue'
...
test.describe('營收分析（端點尚未實作）', () => {
  ...
  // ─── 端點不存在：帶完整查詢參數仍應回傳 404 ───

  test('帶日期與 interval 參數請求仍應回傳 404', async ({ request }) => {
    const res = await apiGet(request, {
  ...
  test('404 錯誤回應應符合 WordPress REST API 標準錯誤格式', async ({ request }) => {
    const res = await apiGet(request, { product_ids: '1' })

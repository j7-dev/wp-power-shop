...
export const ZFormValues = z.object({
		...
		.tuple([
			z
				...
				.refine((val) => val.toString().length === 10, {
					message: '開始時間必須是10位的時間戳',
				})
				.optional(),
			z
				.number()
				.refine((val) => val.toString().length === 10, {
					message: '結束時間必須是10位的時間戳',
				})
				.optional(),
		])
		.optional(),
	...
	// Status
	status: z.enum(
		PRODUCT_STATUS.map((status) => status.value) as [string, ...string[]]
	),
	catalog_visibility: z.enum(
		PRODUCT_CATALOG_VISIBILITIES.map((visibility) => visibility.value) as [
			string,
			...string[],
		]
	),
	...
	stock_status: z.enum(
		PRODUCT_STOCK_STATUS.map((status) => status.value) as [string, ...string[]]
	),

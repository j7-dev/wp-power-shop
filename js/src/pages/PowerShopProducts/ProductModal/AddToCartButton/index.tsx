		...
		mutationOptions: {
			onMutate: (variables) => {
				...
				const variation = JSON.parse(
					variables.variation as string,
				) as TProductVariationAttribute[]
				...
				setCartData((prev) => {
					const findSameItem = prev.items.find(
						(i) =>
							i.id === newCartItem.id &&
							JSON.stringify(i.variation) ===
								JSON.stringify(newCartItem.variation),
					)
	...
	return (
		<>
			...
			<Button
				...
				disabled={
					!qtyAvailable ||
					!canAddToCart(product, selectedVariationId, shopStatus, qtyInCart)
				}
			>
				加入購物車
			</Button>

			...
			onMutate: (variables) => {
				setCartData((prev) => ({
					...
					items: prev.items.filter(
						(item) => item.key !== variables.cart_item_key,
					),
					isMutating: true,
				}))

				const rollBack = cartData.items.find(
					(item) => item.key === variables.cart_item_key,
				)
				return rollBack
			},
				...
				{cartItems.length !== 0 &&
					cartItems.map((item) => {
						...
						return (
							<tr
								key={item.key}
								className={`${item.isMutating ? 'animate-pulse' : ''}`}
							>
								<td>
									<div className="flex">
										<img
				...
				<tr>
					...
					<th>
						<LoadingText
							...
							isLoading={isLoading || cartResult.isFetching}
						/>
					</th>
					<th></th>
				</tr>
			...
			<div className="text-right mb-8">
				<a href={checkoutUrl}>
					<Button
						...
						className="px-12"
						type="primary"
						disabled={cartItems.length === 0 || cartData.isMutating}
					>
						前往結帳
					</Button>
				</a>

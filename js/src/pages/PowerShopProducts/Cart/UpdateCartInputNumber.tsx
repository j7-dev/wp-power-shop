...
const UpdateCartInputNumber: React.FC<{ item: TCartItem }> = ({ item }) => {
	...
	const handleMutate =
		(
			v: number | null | 'INCREMENT' | 'DECREMENT',
		): ((_: TCartData) => TCartData) =>
		(prev: TCartData) => {
...
function getQuantity(
	value: number | null | 'INCREMENT' | 'DECREMENT',
	theItem: TCartItem,
	stockQty: number,
) {
...
function findStockQtyById(id: number) {
	const findProduct = window.appData.products_info.products.find(
		(p) => p.id === id,
	)
	if (findProduct) {
	...
	const allVariations = window.appData.products_info.products

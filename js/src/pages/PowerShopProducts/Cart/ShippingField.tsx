...
const renderItem = (shipping_rate: TShippingRates, currency_minor_unit: number) => {
  const price = getPrice(parseInt(shipping_rate?.price ?? '0', 10), currency_minor_unit)

  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center">{shipping_rate?.name ?? '未知運送方式'}</div>
...
const ShippingField: React.FC<{ cartData: TCart; isLoading: boolean }> = ({ cartData, isLoading }) => {
  ...
  const raw_total_shipping = parseInt(cartData?.totals?.total_shipping || '0', 10)
  ...
  const defaultOption = shipping_rates.find((shipping_rate) => shipping_rate.selected)?.rate_id ?? ''
  ...
  return (
    <tr>
      <th className="text-left pl-4" colSpan={3}>
        ...
        <Select className="min-w-[12rem]" defaultValue={nonOptions[0].value} value={options.length > 0 ? defaultOption : nonOptions[0].value} options={options.length > 0 ? options : nonOptions} onChange={handleChange} loading={isLoading} disabled={options.length === 0} />
        {options.length === 0 && (
      ...
      <th className="text-right">
        <LoadingText width="w-[4rem]" content={`$ ${total_shipping.toLocaleString()}`} isLoading={isLoading || updateIsLoading} />
      </th>
      <th></th>
    </tr>
  )
